...
export class Migration20230501231012 extends Migration {
  ...
  async up(): Promise<void> {
    this.addSql('create table "dashboard_data_source" ("id" serial primary key, "created" timestamptz(0) not null, "updated" timestamptz(0) not null, "name" varchar(255) not null, "url" varchar(255) not null);');
    ...
    this.addSql('alter table "establishment" add constraint "establishment_pkey" primary key ("id");');
  ...
  async down(): Promise<void> {
    ...
    this.addSql('alter table "establishment" add constraint "establishment_pkey" primary key ("cnes");');
